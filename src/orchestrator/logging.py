...
if TYPE_CHECKING:
    from orchestrator.observability.providers.langfuse_client import LangfuseClient
...
def logger_for_module(module_name: str) -> OrchestratorLogger:
    ...
    Example:
        ```python
        from orchestrator.logging import logger_for_module

        logger = logger_for_module(__name__)
        ```
    """
    # Extract the relevant part after 'orchestrator'
    if module_name.startswith("orchestrator."):
        return get_logger(module_name)
    return get_logger(f"orchestrator.{module_name}")
...
def clear_loggers() -> int:
    ...
    Example:
        ```python
        from orchestrator.logging import clear_loggers
