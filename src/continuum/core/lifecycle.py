...
class OrchestratorLifecycle:
    ...
    async def _initialize_clients(self) -> None:
        ...
        if settings.memory_enabled:
            try:
                from continuum.memory import initialize_global_memory
    ...
    def _log_service_configurations(self) -> None:
        ...
        if settings.memory_enabled:
            try:
                from continuum.memory import get_global_memory_client
        ...
        if settings.session_enabled:
            try:
                from continuum.session import get_global_session_client
        ...
        try:
            from continuum.observability import get_provider_manager
    ...
    async def _shutdown_clients(self) -> None:
        ...
        if "temporal" in self._initialized_components:
            try:
                from continuum.temporal import get_worker_manager
        ...
        if "memory" in self._initialized_components:
            try:
                from continuum.memory.client import (
