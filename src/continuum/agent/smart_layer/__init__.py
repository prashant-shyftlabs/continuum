...
from continuum.agent.smart_layer.defaults import (
    CLASSIFIER_SYSTEM_PROMPT,
    MODEL_TIER_DEFAULT_INSTRUCTIONS,
)
from continuum.agent.smart_layer.types import (
    ClassifyOutcome,
    ModelTierResult,
    ProductTier,
    StreamYield,
...
def __getattr__(name: str):
    ...
    if name == "effective_completion_model":
        from continuum.agent.smart_layer.resolve import effective_completion_model

        return effective_completion_model
    if name == "extract_last_user_text":
        from continuum.agent.smart_layer.runner_facade import extract_last_user_text
    ...
    if name == "parse_classifier_json":
        from continuum.agent.smart_layer.json_parse import parse_classifier_json
    ...
    if name == "run_model_tier_turn":
        from continuum.agent.smart_layer.runner_facade import run_model_tier_turn

        return run_model_tier_turn
    if name == "stream_model_tier_turn":
        from continuum.agent.smart_layer.runner_facade import stream_model_tier_turn
